...
comments: true
---
通过[原版](https://github.com/menzi11/BullshitGenerator)项目改为C++版本，基本依照原版内容，几乎没太大改变，侵删。

代码：

```cpp
#include<bits/stdc++.h>
#define Rand(x) x[rand()%(sizeof(x)/8)]
using namespace std;
const string discussion[]=
{
...
const string behind_transition[]=
{
    "这不禁令我深思。 ",
...
string topic;
string change(string s,string x,string y)
{
...
string get_saying()
{
	string Saying=Rand(saying);
	Saying=change(Saying,"曾经说过",Rand(front_transition));
	Saying=change(Saying,"这不禁令我深思",Rand(behind_transition));
	return Saying;
}
string get_discussion()
{
	string sentence=Rand(discussion);
	sentence=change(sentence,"主题",topic);
	return sentence;
}
string add_passage(string passage)
{
	if(passage[passage.size()-1]==' ')
		passage=passage.substr(0,passage.size()-3);
	return "　　"+passage+"。\n"; 
}
int main()
{
	string passage="",article="";
	int length=0,max_length,file;
	srand(time(0));
	cout<<"主题:";
	cin>>topic;
	cout<<"字数:";
	...
	if(file==1)
	{
		cout<<"文件将保存至相同目录的\"article.txt\"内\n\n";
		system("pause");
		freopen("article.txt","w",stdout);
	}
	while(length<max_length*2)
	{
		int random=rand()%100;
		if(random<5&&passage.size()>200)
		{
			passage=add_passage(passage);
			article+=passage;
			passage="";
		}
		else if(random<20)
		{
			string sentence=get_saying();
			length=length+sentence.size();
			passage=passage+sentence;
		}
		else
		{
			string sentence=get_discussion();
			length=length+sentence.size();
			passage=passage+sentence;
		}
	}
